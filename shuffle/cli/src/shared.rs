...
use std::{collections::{BTreeMap, HashMap}, fs, fs::File, io::Write, path::{Path, PathBuf}, str::FromStr, time::Duration, thread, time, io};
...
impl DevApiClient {
    ...
    fn is_execution_successful(json: &Value) -> Result<bool> { //todo: think abuot if this should be here
        json["success"]
            .as_bool()
            .ok_or_else(|| anyhow!("Unable to access success key from json"))
    }

    pub fn get_hash_from_post_txn(json: Value) -> Result<String> {
        Ok(json["hash"].as_str().ok_or_else(|| anyhow!("Unable to access hash key from json"))?.to_string())
    }
}
...
impl Network {
    pub fn new(name: String, json_rpc_url: Url, dev_api_url: Url, faucet_url: Option<Url>) -> Self {
        Self {
            name,
            json_rpc_url,
            dev_api_url,
...
mod test {
    ...
    fn test_print_confirmation_with_success_value() {
        ...
        DevApiClient::confirm_successful_execution(&mut stdout, &successful_txn, good_hash).unwrap();
    ...
    fn test_confirm_is_execution_successful() {
        let successful_txn = get_transactions_by_hash_json_output_success();
        assert_eq!(DevApiClient::is_execution_successful(&successful_txn).unwrap(), true);

        let failed_txn = get_transactions_by_hash_json_output_fail();
        assert_eq!(DevApiClient::xecution_successful(&failed_txn).unwrap(), false);
    }
