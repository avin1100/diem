...
pub fn handle(project_path: &Path) -> Result<()> {
    ...
    send_module_transaction(
        &client,
        &mut new_account,
        project_path,
        &factory,
        shuffle_config.get_network(),
    )?;
    run_deno_test(
        project_path,
        &config,
...
fn send_module_transaction(
    ...
    *new_account.sequence_number_mut() = account_seq_num;
    ...
    let hashes = deploy::send_module_transaction(&compiled_package, new_account, factory, network)?;
    for hash in &hashes {
        deploy::confirm_txn_executed(hash.as_str(), network)?
    }
    Ok(())
}
