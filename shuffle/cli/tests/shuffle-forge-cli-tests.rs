...
fn main() -> Result<()> {
    let tests = ForgeConfig::default().with_admin_tests(&[
        ...
        &TransactionsWithNetworkFlag,
        &TransactionsWithNetworkAndRawFlag,
...
pub struct TransactionsWithNetworkFlag;

impl Test for TransactionsWithNetworkFlag {
    fn name(&self) -> &'static str {
        "shuffle::transactions-with-network-flag"
    }
}

impl AdminTest for TransactionsWithNetworkFlag {
    fn run<'t>(&self, ctx: &mut AdminContext<'t>) -> Result<()> {
        let temp_dir = tempdir()?;
        let base_path = temp_dir.path();
        ...
        let corrected_string = format!("{:#?}", std_out);
        let pretty_string = serde_json::to_string_pretty(std_out.as_str())?;
        assert_eq!(corrected_string, pretty_string);
        assert_modules_appear_in_txns(std_out.as_str(), false);
        assert_eq!(output.status, ExitStatus::from_raw(0));
        Ok(())
    }
}

pub struct TransactionsWithNetworkAndRawFlag;

impl Test for TransactionsWithNetworkAndRawFlag {
    fn name(&self) -> &'static str {
        "shuffle::transactions-with-networks-and-raw-flag"
    }
}

impl AdminTest for TransactionsWithNetworkAndRawFlag {
    fn run<'t>(&self, ctx: &mut AdminContext<'t>) -> Result<()> {
        let temp_dir = tempdir()?;
        let base_path = temp_dir.path();
        let helper = bootstrap_shuffle(ctx, base_path)?;
